...
struct SHttpSession {
  ...
  VideoPlayerDrmLicenseHelper::DrmTypeHelper type;
...
static struct curl_slist* g_SetHttpHeader(
    CURL* pCurl, VideoPlayerDrmLicenseHelper::DrmTypeHelper drm_type,
    const char* cookie, const char* f_pHttpHeader, const char* f_pUserAgent);
...
static DRM_RESULT g_HttpStartTransaction(
    ...
    unsigned f_cbPostData, VideoPlayerDrmLicenseHelper::DrmTypeHelper drm_type,
    const char* cookie, const char* f_pSoapHeader, const char* f_pHttpHeader,
...
DRM_RESULT g_ComposePostData_TZ(SHttpSession* hSession,
  ...
  *dest = '\0';
  ...
  if (extSoapHeaderLen > 0) {
    LOG_DEBUG("[soap header added %d ] %s ", hSession->postDataLen,
              hSession->postData);
  }
...
struct curl_slist* g_SetHttpHeader(
    CURL* pCurl, VideoPlayerDrmLicenseHelper::DrmTypeHelper drm_type,
    const char* cookie, const char* f_pHttpHeader, const char* f_pUserAgent) {
  const char* userAgent = nullptr;
  const char* hdr = nullptr;

  if (drm_type == VideoPlayerDrmLicenseHelper::DRM_TYPE_PLAYREADY) {
    userAgent = DEFAULT_USER_AGENT_PLAYREADY;
    hdr = HTTP_HEADER_PLAYREADY_LICGET;
  } else if (drm_type == VideoPlayerDrmLicenseHelper::DRM_TYPE_WIDEVINE) {
  ...
  if (f_pUserAgent) {
    ...
    LOG_DEBUG("g_SetHttpHeader :  user-agent added to header --- (%s)",
              userAgentString);
  ...
  LOG_DEBUG("g_SetHttpHeader : drm_type(%d), cookie(%s), f_pHttpHeader(%s)",
            drm_type, cookie, f_pHttpHeader);
  ...
  if (f_pHttpHeader) {
    LOG_DEBUG("g_SetHttpHeader :  HttpHeader added to header --- (%s)",
              f_pHttpHeader);
...
int g_CbCurlProgress(void* ptr, double TotalToDownload, double NowDownloaded,
  ...
  if (pCancelRequest) {
    LOG_DEBUG("pCancelRequest : (%d)", *pCancelRequest);

    if (*pCancelRequest) {
      LOG_DEBUG("%s:%d curl works canceled.", __FUNCTION__, __LINE__);
      return 1;
    }
  }
...
DRM_RESULT g_HttpStartTransaction(
  ...
  LOG_INFO("g_HttpStartTransaction : drm_type(%d)", drm_type);
  if (f_pUrl) {
    LOG_INFO("f_pUrl : %s", f_pUrl);
  }

  // 2. Set Header type
  hSession->type = drm_type;
  headers =
      g_SetHttpHeader(pCurl, drm_type, cookie, f_pHttpHeader, f_pUserAgent);
  if (!headers) {
...
DRM_RESULT VideoPlayerDrmLicenseHelper::DoTransactionTZ(
    const char* server_url, const void* challenge_data,
    unsigned long challenge_data_length, unsigned char** response_data,
    unsigned long* response_data_length,
    VideoPlayerDrmLicenseHelper::DrmTypeHelper drm_type, const char* cookie,
    ExtensionCtxTZ* extentsion_context) {
  *response_data = nullptr;
  *response_data_length = 0;

  const char* pUrl = server_url;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (extentsion_context != nullptr) {
      if (extentsion_context->soap_header) {
        pSoapHdr = extentsion_context->soap_header;
      }

      if (extentsion_context->http_header) {
        pHttpHdr = extentsion_context->http_header;
      }

      if (extentsion_context->user_agent) {
        user_agent = extentsion_context->user_agent;
      }

      pCancelRequest = &(extentsion_context->cancel_request);
    }

    dr = g_HttpStartTransaction(pSession, pUrl, challenge_data,
                                challenge_data_length, drm_type, cookie,
                                pSoapHdr, pHttpHdr, user_agent, pCancelRequest);
    if (dr != DRM_SUCCESS) {
    ...
    if (pSession->resCode == 301 || pSession->resCode == 302) {
      ...
      *response_data = pSession->body.iData;
      *response_data_length = pSession->body.iSize;
